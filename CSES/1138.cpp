...
#define int ll
const int z=2e5+5;
int top[z],dep[z],dfn[z],tot,n,m,a[z],son[z],sz[z],fa[z],rk[z];
ll seg[z<<2];
vector<int> v[z];
void dfs(int x){
    sz[x]=1;
...
int que(int l,int r,int ql,int qr,int rt){
    ...
    if(qr>mid) ret+=que(mid+1,r,ql,qr,rs);
    return ret;
}
int qry(int x,int y){
    int ret=0;
    while(top[x]!=top[y]){
        ...
        ret+=que(1,n,dfn[top[x]],dfn[x],1);
        x=fa[top[x]];
    }
    if(dep[x]>dep[y])swap(x,y);
    return ret+que(1,n,dfn[x],dfn[y],1);
}
signed main(){Aincrad
    cin>>n>>m;
    REP1(i,1,n) cin>>a[i];
    ...
    for(int k,s,x;m--;){
        cin>>k>>s;
        if(k==1) cin>>x,mdf(1,n,dfn[s],1,x);
        else cout<<qry(1,s)<<"\n";
    }
    return 0;
}
